...
// Lógica de negocio del recurso Docentes. Extraída tal cual de la ruta original
// (routes/docentesRoute.js) para no alterar ningún comportamiento: mismas
// consultas, mismas respuestas, mismos mensajes y códigos HTTP.
...
// GET /docentes/contratow/:codigo — generar word
async function generarWord(req, res) {
  try {
...
// GET /docentes/contrato/:codigo/:codr — generar pdf
async function generarContratoPdf(req, res) {
  try {
...
// GET /docentes/contratocontra/:codigo/:codr — pdf contratados
async function generarContratoContratadoPdf(req, res) {
  try {
    const jefePractica = await Login.findOne({
      where: { cargo: '1' },
    });
...
// DELETE /docentes/:codigodocentes
async function eliminar(req, res) {
  try {
    const codigo = req.params.codigodocentes;
    // Eliminar el registro de departamento en la base de datos
